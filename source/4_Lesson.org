...
- The code that belongs to the function goes inside curly brackets ={}= just like it did for conditionals (=if,elseif,else=)
...
- Make sure you give your function a sensible name, and don't forget to add the =()= after the name and the ={}= to contain your code.
